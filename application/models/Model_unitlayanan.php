...
class Model_unitlayanan extends CI_Model
{
	...
	public function updateunitlayanan()
   {
      ...
      $data = [
         ...
         'unit' => $unit,
      	 'nama_unit_layanan' => $unit_layanan
      ];
     ...
     function fetch_layanan($id_unit)
   {
      ...
      $this->db->order_by('nama_unit_layanan', 'ASC');
      $query = $this->db->get('tb_unit_layanan');
      $output = '<option value="">Select Unit Layanan</option>';
      foreach($query->result() as $row)
      {
       $output .= '<option value="'.$row->id_unit_layanan.'">'.$row->nama_unit_layanan.'</option>';
      }
      return $output;
   }

   function fetch_layanan_userpic($id_unit_layanan_pic)
   {
      $this->db->where('unit_layanan', $id_unit_layanan_pic);
      $this->db->order_by('nama', 'ASC');
      $query = $this->db->get('tb_user');
      $output = '<option value="">Select User PIC</option>';
      foreach($query->result() as $row)
      {
       $output .= '<option value="'.$row->id_user.'">'.$row->nama.'</option>';
      }
      return $output;
   }
