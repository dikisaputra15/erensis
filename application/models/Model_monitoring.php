...
class Model_monitoring extends CI_Model
{
   ...
   {
      $id_layanan = $this->session->userdata('unit_layanan');
       $hasil=$this->db->query("SELECT tb_utama.*, tb_unit_induk.*, tb_unit.*, tb_unit_layanan.*, tb_penyulang.*, tb_trafo_gi.*, tb_gardu.*, tb_program_kerja.*, tb_tarif.*, tb_daya.*, tb_user.*, tb_status.* from tb_utama join tb_unit_induk on tb_utama.id_unit_induk=tb_unit_induk.id_unit_induk join tb_unit on tb_utama.id_unit=tb_unit.id_unit join tb_unit_layanan on tb_utama.id_layanan_pic=tb_unit_layanan.id_unit_layanan join tb_penyulang on tb_utama.penyulang=tb_penyulang.id_penyulang join tb_trafo_gi on tb_utama.trafo_gi=tb_trafo_gi.id_trafo_gi join tb_gardu on tb_utama.gardu=tb_gardu.id_gardu join tb_program_kerja on tb_utama.jenis_program_kerja=tb_program_kerja.id_program join tb_tarif on tb_utama.id_tarif=tb_tarif.id_tarif join tb_daya on tb_utama.daya=tb_daya.id_daya join tb_user on tb_utama.id_user=tb_user.id_user join tb_status on tb_utama.status=tb_status.id_status where tb_utama.id_layanan_pic='$id_layanan'");
        return $hasil->result();
   }

   public function getAllmonitoringlap()
   {
       $hasil=$this->db->query("SELECT tb_utama.*, tb_unit_induk.*, tb_unit.*, tb_unit_layanan.*, tb_penyulang.*, tb_trafo_gi.*, tb_gardu.*, tb_program_kerja.*, tb_tarif.*, tb_daya.*, tb_user.*, tb_status.* from tb_utama join tb_unit_induk on tb_utama.id_unit_induk=tb_unit_induk.id_unit_induk join tb_unit on tb_utama.id_unit=tb_unit.id_unit join tb_unit_layanan on tb_utama.id_layanan_pic=tb_unit_layanan.id_unit_layanan join tb_penyulang on tb_utama.penyulang=tb_penyulang.id_penyulang join tb_trafo_gi on tb_utama.trafo_gi=tb_trafo_gi.id_trafo_gi join tb_gardu on tb_utama.gardu=tb_gardu.id_gardu join tb_program_kerja on tb_utama.jenis_program_kerja=tb_program_kerja.id_program join tb_tarif on tb_utama.id_tarif=tb_tarif.id_tarif join tb_daya on tb_utama.daya=tb_daya.id_daya join tb_user on tb_utama.id_user=tb_user.id_user join tb_status on tb_utama.status=tb_status.id_status");
        return $hasil->result();
   }

   public function getAllmonitoringbyid($id)
   {
          
      $query = "SELECT tb_utama.*, tb_unit_induk.*, tb_unit.*, tb_unit_layanan.*, tb_penyulang.*, tb_trafo_gi.*, tb_gardu.*, tb_program_kerja.*, tb_tarif.*, tb_daya.*, tb_user.* from tb_utama join tb_unit_induk on tb_utama.id_unit_induk=tb_unit_induk.id_unit_induk join tb_unit on tb_utama.id_unit=tb_unit.id_unit join tb_unit_layanan on tb_utama.id_unit_layanan=tb_unit_layanan.id_unit_layanan join tb_penyulang on tb_utama.penyulang=tb_penyulang.id_penyulang join tb_trafo_gi on tb_utama.trafo_gi=tb_trafo_gi.id_trafo_gi join tb_gardu on tb_utama.gardu=tb_gardu.id_gardu join tb_program_kerja on tb_utama.jenis_program_kerja=tb_program_kerja.id_program join tb_tarif on tb_utama.id_tarif=tb_tarif.id_tarif join tb_daya on tb_utama.daya=tb_daya.id_daya join tb_user on tb_utama.id_user=tb_user.id_user where tb_utama.id_utama='$id'";
      return $this->db->query($query)->row_array();
   }

 	public function insert_monitoring($data, $table)
   {
   ...
   {
      $this->db->delete('tb_utama', ['id_utama' => $id]);
   }
   ...
   public function update_status($id)
   {
	  ...
	  $this->db->update('tb_utama', $data);
   }

    public function getmonitoringbyid($id)
   {
      return $this->db->get_where('tb_utama', ['id_utama' => $id])->row_array();
   }
   ...
   public function updatemonitoring()
   {
      ...
      $total_anggaran_uid = $this->input->post('total_anggaran_uid');
      $total_anggaran_unit = $this->input->post('total_anggaran_unit');
      $total_anggaran_errect = $this->input->post('total_anggaran_errect');
      ...
      $gambar = $_FILES['gambar']['name'];
       ...
       if ($gambar) {
            ...
            if ($this->upload->do_upload('gambar')) {
               ...
               $this->db->set('upload_gambar', $new_image);
            } else {
        ...
        $n_daya_akhir10 = 0;
         ...
         $gain_penyambungan = (($rp_tarif-$bpp)* ($jam_nyala*12)*(($pf*($n_daya_akhir1 - $n_daya_awal1))/1000))
               +(($rp_tarif2-$bpp)* ($jam_nyala*12)*(($pf*($n_daya_akhir2 - $n_daya_awal2))/1000))
               +(($rp_tarif3-$bpp)* ($jam_nyala*12)*(($pf*($n_daya_akhir3 - $n_daya_awal3))/1000))
               +(($rp_tarif4-$bpp)* ($jam_nyala*12)*(($pf*($n_daya_akhir4 - $n_daya_awal4))/1000))
               +(($rp_tarif5-$bpp)* ($jam_nyala*12)*(($pf*($n_daya_akhir5 - $n_daya_awal5))/1000))
               +(($rp_tarif6-$bpp)* ($jam_nyala*12)*(($pf*($n_daya_akhir6 - $n_daya_awal6))/1000))
               +(($rp_tarif7-$bpp)* ($jam_nyala*12)*(($pf*($n_daya_akhir7 - $n_daya_awal7))/1000))
               +(($rp_tarif8-$bpp)* ($jam_nyala*12)*(($pf*($n_daya_akhir8 - $n_daya_awal8))/1000))
               +(($rp_tarif9-$bpp)* ($jam_nyala*12)*(($pf*($n_daya_akhir9 - $n_daya_awal9))/1000))
               +(($rp_tarif10-$bpp)* ($jam_nyala*12)*(($pf*($n_daya_akhir10 - $n_daya_awal10))/1000));
         ...
         $gain2 = (720*$loss_saving)+(720*$ENS_saving)+
                (($rp_tarif-$bpp)* ($jam_nyala)*(($pf*($n_daya_akhir1 - $n_daya_awal1))/1000))
               +(($rp_tarif2-$bpp)* ($jam_nyala)*(($pf*($n_daya_akhir2 - $n_daya_awal2))/1000))
               +(($rp_tarif3-$bpp)* ($jam_nyala)*(($pf*($n_daya_akhir3 - $n_daya_awal3))/1000))
               +(($rp_tarif4-$bpp)* ($jam_nyala)*(($pf*($n_daya_akhir4 - $n_daya_awal4))/1000))
               +(($rp_tarif5-$bpp)* ($jam_nyala)*(($pf*($n_daya_akhir5 - $n_daya_awal5))/1000))
               +(($rp_tarif6-$bpp)* ($jam_nyala)*(($pf*($n_daya_akhir6 - $n_daya_awal6))/1000))
               +(($rp_tarif7-$bpp)* ($jam_nyala)*(($pf*($n_daya_akhir7 - $n_daya_awal7))/1000))
               +(($rp_tarif8-$bpp)* ($jam_nyala)*(($pf*($n_daya_akhir8 - $n_daya_awal8))/1000))
               +(($rp_tarif9-$bpp)* ($jam_nyala)*(($pf*($n_daya_akhir9 - $n_daya_awal9))/1000))
               +(($rp_tarif10-$bpp)* ($jam_nyala)*(($pf*($n_daya_akhir10 - $n_daya_awal10))/1000));
         
         if ($gain2 == 0) {
            $payback = 0;
         }else{
           $payback = ($nilai_investasi-$biaya_penyambungan)/(($ttl-$bpp)*($gain2))/12;   
         }
         ...
         if($payback <0){$payback=0;}
          ...
          $this->db->update('tb_utama', $data, ['id_utama' => $this->input->post('id_utama')]);
   }

	public function updatemonitoring1()
   {
      $unit_induk = $this->input->post('unit_induk');
      $unit = $this->input->post('unit');
      $unit_layanan = $this->input->post('unit_layanan');
      ...
      $nilai_kontribusi = $this->input->post('nilai_kontribusi');
      $npv = $this->input->post('npv');
      $irr = $this->input->post('irr');
      $payback = $this->input->post('payback');
      $status = $this->input->post('status');
      $tgl_input = date('Y-m-d');

      $file1 = $_FILES['rab']['name'];
      $file2 = $_FILES['kkp']['name'];
      ...
      if($file1 != ""){
         ...
         $rab = $mac . time().'.pdf';
            ...
            if ( ! $this->upload->do_upload('rab'))
            {
                     ...
                     $this->load->view('monitoring/monitoring', $error);
            ...
            if ( ! $this->upload->do_upload('kkp'))
            {
                  ...
                  $data = [
                        ...
                        'segmen' => $id_segmen,
                        'gardu' => $id_gardu,
                        'jenis_pekerjaan' => $id_jp,
                        'nilai_investasi' => $nilai_investasi,
                        'jenis_kontribusi' => $id_jk,
                        'nilai_kontribusi' => $nilai_kontribusi,
                        'npv' => $npv,
                        'irr' => $irr,
                        'payback' => $payback,
                        'status' => $status,
   ...
   public function get_fetch_detail($Data)
   {
       $query = "SELECT tb_utama.*, tb_unit_induk.*, tb_unit.*, tb_unit_layanan.*, tb_penyulang.*, tb_trafo_gi.*, tb_gardu.*, tb_program_kerja.*, tb_tarif.*, tb_daya.*, tb_user.* from tb_utama join tb_unit_induk on tb_utama.id_unit_induk=tb_unit_induk.id_unit_induk join tb_unit on tb_utama.id_unit=tb_unit.id_unit join tb_unit_layanan on tb_utama.id_unit_layanan=tb_unit_layanan.id_unit_layanan join tb_penyulang on tb_utama.penyulang=tb_penyulang.id_penyulang join tb_trafo_gi on tb_utama.trafo_gi=tb_trafo_gi.id_trafo_gi join tb_gardu on tb_utama.gardu=tb_gardu.id_gardu join tb_program_kerja on tb_utama.jenis_program_kerja=tb_program_kerja.id_program join tb_tarif on tb_utama.id_tarif=tb_tarif.id_tarif join tb_daya on tb_utama.daya=tb_daya.id_daya join tb_user on tb_utama.id_user=tb_user.id_user where id_utama='$Data'";
      return $this->db->query($query);
   }

   public function get_fetch_histori($Data)
   {
       $query = "SELECT tb_histori.*, tb_unit_induk.*, tb_unit.*, tb_unit_layanan.*, tb_user.*, tb_status.* from tb_histori join tb_unit_induk on tb_histori.id_unit_induk=tb_unit_induk.id_unit_induk join tb_unit on tb_histori.id_unit=tb_unit.id_unit join tb_unit_layanan on tb_histori.id_unit_layanan=tb_unit_layanan.id_unit_layanan join tb_user on tb_histori.id_user=tb_user.id_user join tb_status on tb_histori.id_status=tb_status.id_status where id_utama='$Data' ORDER BY tb_histori.tanggal ASC";
